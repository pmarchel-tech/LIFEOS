...
const getLatestUpdateDetails = (rec) => {
  ...
  if (Array.isArray(rec.updates) && rec.updates.length > 0) {
    const sorted = [...rec.updates].sort((a, b) => {
  ...
  if (dateStr) {
    try {
      ...
      if (parts.length >= 3) {
        ...
        if (mIdx >= 0 && mIdx < 12) {
          dateFormatted = `${MONTH_NAMES[mIdx]}-${dStr}`;
        }

        const targetDate = new Date(y, mIdx, d);
        const now = new Date();
      ...
      console.warn('Error parsing update date:', e);
    }
  }

  return { dateStr, dateFormatted, text, diffDays };
};
    ...
    try {
      ...
      if (saved) {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          let complete = [...parsed];
          // Ensure updateAge is positioned right before latestUpdate if not present
          if (!complete.includes('updateAge')) {
            const latestIdx = complete.indexOf('latestUpdate');
            if (latestIdx !== -1) {
              complete.splice(latestIdx, 0, 'updateAge');
            } else {
              complete.push('updateAge');
            }
          }
      ...
      <table className="w-full text-left text-xs border-collapse table-fixed">
              ...
              if (colKey === 'updateAge') {
                return (
                  <th
                    ...
                    {...dragProps}
                    style={{ width: `${colWidth}px`, minWidth: `${colWidth}px` }}
                    onClick={() => onSort && onSort('updateAge')}
                    className={`${commonThClass} hover:bg-slate-100 dark:hover:bg-slate-700/60 ${
                      sortField === 'updateAge' ? 'bg-blue-50/60 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400' : ''
                    }`}
                    title="Drag untuk memindahkan kolom • Klik untuk mengurutkan Selisih Hari sejak update terakhir"
                  >
                    <div className="flex items-center justify-between gap-1">
                      <div className="flex items-center gap-1 min-w-0 truncate">
                        <GripVertical className="w-3 h-3 text-slate-300 dark:text-slate-600 opacity-0 group-hover:opacity-100 shrink-0" />
                        <span className="truncate">Update age</span>
                      </div>
                      <SortIndicator field="updateAge" />
                    </div>
                    {resizerHandle}
                  </th>
                );
        ...
        <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
          {Object.entries(groupedRecords).map(([groupName, groupItems]) => {
            ...
            return (
              <React.Fragment key={groupName}>
                ...
                >
                  <td colSpan={columnOrder.length + 2} className="py-2 px-3">
                    <div className="flex items-center justify-between">
                        ...
                        <React.Fragment key={rec.id}>
                          ...
                          >
                            ...
                            {columnOrder.map(colKey => {
                              ...
                              if (colKey === 'updateAge') {
                                const upd = getLatestUpdateDetails(rec);
                                return (
                                  <td
                                    key="updateAge"
                                    onClick={() => onOpenRecord && onOpenRecord(rec)}
                                    className="py-2 px-2.5 border-r border-slate-100 dark:border-slate-800/80 whitespace-nowrap cursor-pointer hover:bg-blue-50/50 dark:hover:bg-blue-900/20 transition"
                                    style={{ width: `${columnWidths.updateAge || 115}px`, maxWidth: `${columnWidths.updateAge || 115}px`, overflow: 'hidden' }}
                                    title={upd.dateStr ? `Update terakhir: ${upd.dateStr} (${upd.diffDays === 0 ? 'Hari ini' : `${upd.diffDays} hari yang lalu`})` : 'Belum ada riwayat update'}
                                  >
                                    {upd.diffDays === null ? (
                                      <span className="text-slate-400 dark:text-slate-500 text-[11px] font-mono pl-1">-</span>
                                    ) : (
                                      <span
                                        className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold tracking-tight whitespace-nowrap ${
                                          upd.diffDays === 0
                                            ...
                                            : upd.diffDays === 1
                                            ? 'bg-emerald-50/70 text-emerald-600 dark:bg-emerald-950/40 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-900/40'
                                            : upd.diffDays <= 4
                                      ...
                                      >
                                        ...
                                        <span>{upd.diffDays === 0 ? 'Hari ini' : `${upd.diffDays} hr lalu`}</span>
                                      </span>
                                    )}
                                  </td>
                                );
                              ...
                              if (colKey === 'latestUpdate') {
                                const upd = getLatestUpdateDetails(rec);
                                return (
                                  <td key="latestUpdate" className="py-2 px-3 border-r border-slate-100 dark:border-slate-800/80 text-slate-600 dark:text-slate-300 truncate" style={{ width: `${columnWidths.latestUpdate || 240}px`, maxWidth: `${columnWidths.latestUpdate || 240}px`, overflow: 'hidden' }}>
                                    <div
                                      onClick={() => onOpenRecord && onOpenRecord(rec)}
                                      className="cursor-pointer hover:text-blue-600 dark:hover:text-blue-400 transition flex items-center justify-between gap-1.5 group/note"
                                      title={upd.text ? (upd.dateFormatted ? `[${upd.dateFormatted}] ${upd.text}` : upd.text) : 'Buka Side Peek untuk melihat riwayat atau menambah update'}
                                    >
                                      {upd.text ? (
                                        <span className="truncate flex items-center gap-1.5">
                                          {upd.dateFormatted && (
                                            <span className="shrink-0 font-mono text-[10px] font-bold text-slate-700 dark:text-slate-200 bg-slate-100 dark:bg-slate-800 px-1 py-0.2 rounded border border-slate-200 dark:border-slate-700">
                                              [{upd.dateFormatted}]
                                            </span>
                                          )}
                                          <span className="truncate">{upd.text}</span>
                                        </span>
                                      ) : (
                                        <span className="text-slate-400 italic text-[11px]">No update</span>
                                      )}
                          ...
                          {!isSubCollapsed && inlineSubTaskParentId === rec.id && (
                            <tr className="bg-indigo-50/50 dark:bg-indigo-950/30 border-b border-indigo-200 dark:border-indigo-800/60 animate-in fade-in">
                              ...
                              <td colSpan={columnOrder.length + 1} className="py-2 px-3 pl-8">
                                <form
                                  onSubmit={(e) => {
                                    e.preventDefault();
                                ...
                                >
                                  ...
                                  {columnOrder.map(colKey => {
                                    ...
                                    if (colKey === 'dueDate') {
                                     ...
                                     if (colKey === 'updateAge') {
                                       const upd = getLatestUpdateDetails(sub);
                                       return (
                                         <td
                                           key="updateAge"
                                           onClick={() => onOpenRecord && onOpenRecord(sub)}
                                           className="py-2 px-2.5 border-r border-slate-100 dark:border-slate-800/80 whitespace-nowrap cursor-pointer hover:bg-indigo-50/50 dark:hover:bg-indigo-900/20 transition"
                                           style={{ width: `${columnWidths.updateAge || 115}px`, maxWidth: `${columnWidths.updateAge || 115}px`, overflow: 'hidden' }}
                                           title={upd.dateStr ? `Update terakhir: ${upd.dateStr} (${upd.diffDays === 0 ? 'Hari ini' : `${upd.diffDays} hari yang lalu`})` : 'Belum ada riwayat update'}
                                         >
                                           {upd.diffDays === null ? (
          ...
          <tr className="bg-slate-100/50 dark:bg-slate-800/40 border-t-2 border-dashed border-slate-200 dark:border-slate-700">
            <td colSpan={columnOrder.length + 2} className="py-2.5 px-4">
              {isAddingNewCategory ? (
